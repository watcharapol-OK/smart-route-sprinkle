...
if df is not None and not df.empty:
    ...
    st.sidebar.caption("100% = 4,160 ถัง/เดือน (ระบบจะพยายามเกลี่ยงานให้ได้ตาม % ที่ตั้งไว้ทุกคัน)")

    target_pcts = {}
    for t in available_trucks:
        if t == base_truck: continue
        target_pcts[t] = st.sidebar.slider(f"เป้าหมายของรถ {t} (%)", min_value=0, max_value=120, value=95, step=1, on_change=reset_results)

    target_pcts[new_truck_name] = st.sidebar.slider(f"เป้าหมายของรถสายใหม่ {new_truck_name} (%)", min_value=10, max_value=120, value=100, step=1, on_change=reset_results)
    ...
    # ฟังก์ชันแยกวันจัดส่งจากข้อความ
    ...
    # ---------------------------------------------------------
    # สมองกลหลัก: Proportional Spatial Allocation (จาก Colab ที่คุณโอยืนยันว่าใช้งานได้ดี)
    # ---------------------------------------------------------
    ...
    if st.sidebar.button("🚀 ประมวลผลตัดสายส่ง", use_container_width=True):
        ...
        try:
            with open("truck.jpg", "rb") as image_file:
                encoded_string = base64.b64encode(image_file.read()).decode()
            loader_html = f'''<div class="custom-truck-loader"><img src="data:image/jpeg;base64,{encoded_string}" alt="รถกำลังวิ่ง..."><br>กำลังประมวลผลจัดสรรงานตามเปอร์เซ็นต์... 🚚💨</div>'''
        except FileNotFoundError:
        ...
        daily_matrix = get_daily_vols(res_df)
    ...
    if 'result_df' in st.session_state:
        ...
        with col2:
            st.markdown("**หลังปรับโครงสร้าง (ตรงตามเป้าหมายของแต่ละคัน)**")
        ...
        max_daily_load = max([row['โหลดสูงสุด (ถัง/วัน)'] for row in daily_summary])
        if max_daily_load > 190:
            st.error(f"🚨 **แจ้งเตือน:** โหลดรายวันสูงสุดบางวันสูงถึง {max_daily_load} ถัง/วัน (เกินเพดาน 190 ถัง แนะนำให้ปรับลดเปอร์เซ็นต์เป้าหมายหรือเพิ่มรถเพื่อเกลี่ยงาน)")
        elif max_daily_load > 156:
            st.warning(f"⚠️ โหลดรายวันสูงสุดบางวันอยู่ที่ {max_daily_load} ถัง/วัน (อยู่ในเกณฑ์อนุโลมวิ่งเที่ยว 3 ไม่เกิน 190 ถัง)")
        else:
            st.success("✅ โหลดรายวันอยู่ในโซนมาตรฐานคุ้มค่า (<= 156 ถัง/วัน)")
        ...
        if selected_view == "แสดงทั้งหมด (แยกสีตามเบอร์รถ)":
            ...
            if selected_view == new_truck_name and base_truck == "(ไม่มี - เพิ่มรถคันใหม่กระจายงาน)":
                map_df_before = pd.DataFrame(columns=df.columns)
            else:
                map_df_before = df[df[truck_col].astype(str) == (base_truck if selected_view == new_truck_name else selected_view)]
        ...
        with map_col2:
            st.markdown("<div style='text-align:center; color:#002D62; font-weight:bold;'>โซนการวิ่งสายใหม่ (Proportional Target Allocation)</div>", unsafe_allow_html=True)
        ...
        st.markdown("---")
        st.markdown("<div style='text-align:center; margin-bottom: 10px;'><b>📌 ดาวน์โหลดข้อมูลสรุปผลเพื่อใช้งานใน Excel</b></div>", unsafe_allow_html=True)
        ...
        with col_btn2:
            st.download_button(
                ...
                data=csv_bytes,
                file_name='sprinkle_proportional_route_result.csv',
                mime='text/csv',
